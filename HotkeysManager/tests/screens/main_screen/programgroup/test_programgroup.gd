...
func test_can_delete_programgroup() -> void:
    ...
    @warning_ignore("unsafe_call_argument")
    assert_null(programgroup._db.select_value("programgroup", "programgroup_id", "programgroup_id=?", [programgroup._programgroup_id]))
...
func test_cannot_add_unavailable_programs() -> void:
    ...
    check_has_all_programs(["CLion", "Visual Studio", "Visual Studio Code", "Obsidian"])
    assert_engine_error("FOREIGN KEY constraint failed")
